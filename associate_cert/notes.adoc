...
A common pattern for loops involves you initializing a variable before the loop, checking its value in the condition of a while loop, and somehow incrementing or changing that variable inside the loop on each iteration. A `for` loop just puts those things in one place, separated with semicolons. 

For example, to print the numbers from 1  to 10 you would write:
[source,java]
----
for(int i=1;i<=10;++i)
...
The enhanced for loop is used to iterate over a collection, like an array.
[source,java]
----
...
* _syntax_ errors will be caught by the compiler, your code doesn't compile.
* _logic_ errors cause your program to behave in a way you didn't intend it. The program will compile, but it will produce the wrong results.

=== Use exception handling

* Java supports structured exception handling. Any method can raise an _exception_, and whoever calls that method can _catch_ that exception and handle it. 
* If an exception is raised and there's no exception handler, the program will print information about the exception and stop.
* Java has two types of exceptions, _runtime_ or _unchecked_ exceptions, which are supposed to be due to truly exceptional circumstances, which cannot be handled well, and _checked_ exceptions, which signal an input or logical error in your application, and which you should catch.
* Checked exceptions _have_ to be either caught or declared to be thrown in your code.

=== Handle common exceptions thrown

You can define your own exception classes, inheriting from Exception or one of its subclasses. For example:

[source,java]
----
include::com/okaram/samples/ExceptionSamples.java[lines=5..9]
----

or, extending RuntimeException

[source,java]
----
include::com/okaram/samples/ExceptionSamples.java[lines=12..16]
----

And you could throw an exception with the `throw` keyword. For example:
[source,java]
----
include::com/okaram/samples/ExceptionSamples.java[lines=21..28]
----

=== Use try and catch blocks
To catch an exception, you enclose the lines that could cause an exception in a `try` block, and then add `catch` clauses. For example:
[source,java]
----
include::com/okaram/samples/ExceptionSamples.java[lines=31..41]
----
...
* In java you can declare array variables, and the initialize them, either with a constant or by calling new.

[source,java]
----
include::com/okaram/samples/ArraySamples.java[lines=6..10]
----
